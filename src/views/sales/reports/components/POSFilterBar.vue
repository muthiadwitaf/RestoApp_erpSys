<template>
  <div class="pfb-wrapper">
    ...
    <div class="pfb-fields">
      <div class="pfb-field">
        ...
        <input type="date" class="pfb-input" :value="modelFilters.date_from"
               @change="set('date_from', $event.target.value)" id="pfb-date-from" />
      </div>
      <div class="pfb-field">
        <label class="pfb-label">Sampai</label>
        <input type="date" class="pfb-input" :value="modelFilters.date_to"
               @change="set('date_to', $event.target.value)" id="pfb-date-to" />
      </div>
      <div class="pfb-field" v-if="branches.length > 1">
        ...
        <select class="pfb-input" :value="modelFilters.branch_id"
                @change="set('branch_id', $event.target.value)" id="pfb-branch">
          <option value="">Semua Cabang</option>
          <option v-for="b in branches" :key="b.uuid" :value="b.uuid">{{ b.name }}</option>
        </select>
      </div>
      <div class="pfb-field">
        <label class="pfb-label">Kasir</label>
        <select class="pfb-input" :value="modelFilters.cashier_id"
                @change="set('cashier_id', $event.target.value)" id="pfb-cashier">
          <option value="">Semua Kasir</option>
          <option v-for="c in cashiers" :key="c.cashier_id" :value="c.cashier_id">
            {{ c.cashier_name }}
          </option>
        </select>
      </div>
    ...
    <div class="pfb-actions">
      <button class="pfb-btn-load" @click="$emit('submit')" id="pfb-btn-tampilkan">
        <i class="bi bi-search me-2"></i>Tampilkan
      </button>
      <button class="pfb-btn-pdf" @click="$emit('export-pdf')" id="pfb-btn-pdf">
